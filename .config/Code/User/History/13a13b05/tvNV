...
#colores wal?
if [[ "$TERM" != "dumb" && -n "$DISPLAY" && -f "$BG_IMAGE" ]]; then
    if [[ "$(cat ~/.cache/wal/wal 2>/dev/null)" != "$BG_IMAGE" ]]; then
        wal -i "$BG_IMAGE" &> /dev/null
    fi
...
# Aliases can be placed here or in the $ZSH_CUSTOM folder, with .zsh extension. Examples:
...
alias maxpeso='du -ah | sort -hr | head -n 10'
